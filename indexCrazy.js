...
function updateBall() {
  ...
  var ballAI = 0.7; //0 = noise movement 1 = circle

  //change speed when y is closer to 0
  var slowSpeed = 0.00005;
  var fastSpeed = 0.005;
  var speedMap = map(ball.y, 0, canvasSize, slowSpeed, fastSpeed); /// not sure need this

  //change radius to play with 3d
  ball.r = (radiusChange + (ball.y * 0.2)) - 38;

  var xNoise = map(noise(frameCount*fastSpeed), 0, 1, 0, canvasSize-0);
  var yNoise = map(noise(frameCount*speedMap), 0, 1, 0, canvasSize-0);

  // create a changing angle for rotation
  var rotation = frameCount*(speedMap*2);
  ...
  var yBallCircle = orbit * sin(rotation)+300;
